...
describe('Hue API #lights', () => {
  ...
  describe('#getAll()', () => {
    ...
    it('should find some', async () => {
      ...
      expect(results.lights).to.have.length.greaterThan(20);
      ...
      function testBriInc(initialBri, incVal, expectedBri) {
        return async () => {
          const id = testValues.testLightId
            ...
            , initialResult = await hue.lights.setLightState(id, initialState)
            , result = await hue.lights.setLightState(id, incState)
            , finalLightState = await hue.lights.getLightState(id)
          ;
