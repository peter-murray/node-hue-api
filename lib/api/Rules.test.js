...
describe('Hue API #rules', () => {
  ...
  after(async () => {
    if (hue && testOpenCloseSensor) {
      try {
        ...
        console.error(`Failed to remove CLIP Sensor, ${testOpenCloseSensor.toString()}, error: ${err}`);
      }
    }
  });
  ...
  describe('#createRule()', () => {
    ...
    it('should create a new rule', async () => {
      ...
      rule.addCondition(model.ruleConditions.sensor(testOpenCloseSensor).when('open').changed());
      rule.addAction(model.ruleActions.light(testValues.hueLightId).withState(new LightState().on()));
  ...
  describe('#deleteRule()', () => {
    ...
    beforeEach(async () => {
      ...
      rule.recycle = true;
      rule.addCondition(model.ruleConditions.sensor(testOpenCloseSensor).when('open').equals(true));
      rule.addAction(model.ruleActions.light(testValues.hueLightId).withState(new LightState().on()));
  ...
  describe('#updateRule()', () => {
    ...
    beforeEach(async () => {
      ...
      newRule.addCondition(model.ruleConditions.sensor(testOpenCloseSensor).when('open').equals(true));
      newRule.addAction(model.ruleActions.light(testValues.hueLightId).withState(new LightState().on()));
    ...
    it('should update conditions', async () => {
      ...
      rule.addCondition(model.ruleConditions.sensor(testOpenCloseSensor).when('open').equals(false));
    ...
    it('should add an action', async () => {
      rule.resetActions();
      rule.addAction(model.ruleActions.light(0).withState(new LightState().on(false)));
