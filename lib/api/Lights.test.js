...
describe('Hue API #lights', () => {
  ...
  describe('#getAll()', () => {
    ...
    it('should find some', async () => {
      ...
      //TODO this is a pointless test now that we use objects to model the data
      expect(light.getHuePayload()).to.have
  ...
  describe('#rename()', () => {
    ...
    const renameLightId = 2;

    let originalName;

    beforeEach(async () => {
      const light = await hue.lights.getLightById(renameLightId);
      originalName = light.name;
    });

    afterEach(async() => {
      if (originalName) {
        await hue.lights.rename(renameLightId, originalName);
      }
    });

    it('should rename a light', async () => {
      const newName = 'Lounge Living Color'
        , result = await hue.lights.rename(renameLightId, newName)
        , actual = await hue.lights.getLightAttributesAndState(renameLightId);

      expect(result).to.be.true;

      expect(actual).to.have.property('id', renameLightId);
    ...
    it('should error is name is too long',  async () => {
      ...
      try {
        await hue.lights.rename(renameLightId, newName);
