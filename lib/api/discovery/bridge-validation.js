...
const FRIENDLY_NAME = /<friendlyName>(.*)<\/friendlyName/
  ...
  , IP_ADDRESS = /<URLBase>http[s]?:\/\/(.*):.*<\/URLBase/
  , SPEC_VERSION = /<specVersion>.*<major>(.*)<\/.*<minor>(.*)<\/.*<\/specVersion/s
  , ICON_LIST = /<iconList>(.*)<\/iconList>/s
  , ICON_MIME_TYPE = /<mimetype>(.*)<\/mimetype>/s
  , ICON_HEIGHT = /<height>(.*)<\/height>/s
  , ICON_WIDTH = /<width>(.*)<\/width>/s
  , ICON_DEPTH = /<depth>(.*)<\/depth>/s
  , ICON_URL = /<url>(.*)<\/url>/s
;
