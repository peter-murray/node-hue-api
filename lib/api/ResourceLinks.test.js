...
describe('Hue API #resourceLinks', () => {
  ...
  describe('#get()', () => {
    ...
    it('should get a resource link that exists', async () => {
      const allResourceLinks = await hue.resourceLinks.getAll()
        , targetResourceLink = allResourceLinks[allResourceLinks.length - 1]
        , resourceLinkId = targetResourceLink.id
      ;

      const resourceLink = await hue.resourceLinks.get(resourceLinkId);
      expect(model.isResourceLinkInstance(resourceLink)).to.be.true;
      expect(resourceLink).to.have.property('id').to.equal(resourceLinkId);
  ...
  describe('#updateResourceLink()', () => {
    ...
    it('should update the name of a resource link', async () => {
      ...
      const resourceLink = await hue.resourceLinks.get(existingResourceLink.id);
