...
describe('Hue API #groups', () => {
  ...
  describe('#get()', () => {
    ...
    it('should get a room group', async () => {
      const result = await hue.groups.get(existingGroup.id);
    ...
    it('should fail to resolve a group for an invalid id number', async () => {
      try {
        const result = await hue.groups.get(65534);
    ...
    it('should fail to resolve a group for an invalid id as a string', async () => {
      try {
        ...
        expect(err.message).to.contain('not a parsable number value');
      }
    });
  });
  ...
  describe('#getByName()', () => {
    ...
    it('should get an existing group', async () => {
      const allGroups = await hue.groups.getAll()
        ...
        , groups = await hue.groups.getByName(targetGroupName)
      ;
  ...
  describe('#deleteGroup()', () => {
    ...
    beforeEach('createGroup group to be deleted', async () => {
      const result = await hue.groups.createGroup(groupName, GROUP_LIGHTS);
  ...
  describe('#updateAttributes()', () => {
    ...
    //TODO need to deal with tests for updates for room class

    describe('groups', () => {

      const initialGroupName = 'updateGroupTest'
        , initialGroupLights = GROUP_LIGHTS
      ;
      ...
      beforeEach('createGroup group for update', async () => {
        ...
        console.log(JSON.stringify(result.getHuePayload(), null, 2));
      });
      ...
      //TODO validate that we cannot change the class attribute
    });
  ...
  describe('#setGroupState()', () => {
    ...
    beforeEach('createGroup group for setState', async () => {
      const result = await hue.groups.createGroup('groupStateTest', GROUP_LIGHTS);
      groupId = result.id;
    });
