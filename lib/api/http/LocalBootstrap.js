...
const url = require('url')
  ...
  // , httpClient = require('./HttpClientAxios')
...
const INITIAL_HTTPS_AGENT = new https.Agent({rejectUnauthorized: false});
        ...
        return sslCertificate.get(hostname)
          ...
          .then(agent => {
            const apiBaseUrl = `${baseUrl}/api`
              , transport = new Transport(username, httpClient.create({baseURL: apiBaseUrl, httpsAgent: agent}))
              , config = {
                remote: false,
                baseUrl: apiBaseUrl,
                clientkey: clientkey,
