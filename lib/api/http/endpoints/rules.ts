...
import { Rule } from '@peter-murray/hue-bridge-model/lib/esm/model';
...
function createAllRulesResult(result): Rule[] {
  let rules = [];
...
function createGetRuleResult(data, requestParameters): Rule {
  const id = RULE_ID_PLACEHOLDER.getValue(requestParameters);
  return model.createFromBridge('rule', id, data);
}

function validateRuleDeletion(result): boolean {
...
function validateRulePayloadForBridge(rule): void {
  if (!rule.name) {
