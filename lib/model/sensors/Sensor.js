...
module.exports = class Sensor extends BridgeObject {
  ...
  constructor(configAttributes, stateAttributes, id, data) {
    const stateAttribute = parameters.object({
        name: 'state',
        types: Sensor.mergeAttributes(COMMON_STATE_ATTRIBUTES, stateAttributes)
      })
      , configAttribute = parameters.object({
        name: 'config',
        types: Sensor.mergeAttributes(COMMON_CONFIG_ATTRIBUTES, configAttributes)
      })
      , allAttributes = Sensor.mergeAttributes(COMMON_ATTRIBUTES, stateAttribute, configAttribute)
    ;
